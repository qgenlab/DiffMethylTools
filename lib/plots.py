...
>>>>>>> 5c754e21216943eb7fd99182e3a2b0c183256976
    ...
    def match_region_annotation(self, regions_df: InputProcessor.data_container, bed_file: InputProcessor.data_container, name: str = "match_region_annotation", annotation_or_region: str = "region"):
        """
        ...
        for _, region in regions_df.iterrows():
            matched_annotations = annotation_df[
                (annotation_df['chr'] == region['chrom']) &
            ...
            if annotation_or_region == "annotation":
                ...
                for cat in set(all_categories):
                    if cat in special_types:
                        ...
                        this_counts_1[cat] += count_cat
                        key = 'Gene_Body' if 'Gene_' in cat else cat
                        this_counts_2[key] += count_cat
                if len(all_repeat) == 0:
                    this_counts_3['No_repeat'] += 1
                else:
                    for rep_c in set(all_repeat):
                        this_counts_3[rep_c] += all_repeat.count(rep_c)
    ...
    def match_position_annotation(self, regions_df: InputProcessor.data_container, bed_file: InputProcessor.data_container, name:str="match_position_annotation"):
        """
        ...
        self.__annotation_chart(total_counts_1, "Fraction of Occurrences: Gene Intron, Gene Exon, IG, and ENCODE Types", name+'_inex2.png')
        self.__annotation_chart(total_counts_2, "Fraction of Occurrences: Gene Body, IG, and ENCODE Types", name+'_body2.png')
        self.__annotation_chart(total_counts_3, "", name+'_repeat2.png')
        self.__annotation_chart(total_counts_4, "", name+'_epic2.png')
        self.__annotation_chart(total_counts_p4, "", name+'_epicP2.png')
        return [pd.DataFrame.from_dict(list(dict(total_counts_1).items())), pd.DataFrame.from_dict(list(dict(total_counts_2).items())), pd.DataFrame.from_dict(list(dict(total_counts_3).items())), pd.DataFrame.from_dict(list(dict(total_counts_4).items())), pd.DataFrame.from_dict(list(dict(total_counts_p4).items())), pd.DataFrame.from_dict(list(dict(total_gene).items()))]
    def __annotation_chart(self, data, title, fig_name, keep_small=False):
        """
